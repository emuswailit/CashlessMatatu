...
const PayoutsAirtelScreen = ({ route, navigation }) => {
    ...
    navigation.setOptions({
      ...
      headerTitle: (props) => (
        <Text {...props} style={{ color: "white", ...FONTS.body3 }}>
          Payouts to Airtel Money - {vehicle.title}
        </Text>
      ),
      headerStyle: {
  ...
  useEffect(() => {
    if (
      ...
      processPayoutsMpesaApi.data.collection_to_airtel
    ) {
      setref(processPayoutsMpesaApi.data.collection_to_airtel["ref"]);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      airtel_account_number: "",
      amount: 0,
    },

    validationSchema: Yup.object({
      airtel_account_number: Yup.string()
        .min(10, "Too short")
        .required("Airtel phone number"),
      amount: Yup.number().min(1, "Enter 1 or more").required("Amount"),
    }),
    onSubmit: (values, { resetForm }) => {
      console.log("VALZ", values);
      const data = {
        action: "VehicleCollectionToAirtel",
        vehicle_registration: vehicle.title,
        amount: formik.values.amount,
        airtel_account_number: formik.values.airtel_account_number,
        airtel_account_reference: formik.values.airtel_account_reference,
      };
  ...
  return processAuthorizeTransactionApi.loading ||
    ...
    <SafeAreaView className="flex-1 p-2 bg-gray-100">
      ...
      <View>
        ...
        <View className="mt-2">
          <FloatingLabelInput
            id="airtel_account_number"
            name="airtel_account_number"
            label="Airtel number to pay to"
            onChangeText={(value) =>
              formik.setFieldValue("airtel_account_number", value)
            }
            value={formik.values.airtel_account_number}
            inputMode="tel"
          />
          <ErrorMessage
            error={formik.errors.airtel_account_number}
            visible={formik.touched.airtel_account_number}
          />
        </View>

        <View className="mt-2">
          <FloatingLabelInput
            id="airtel_account_reference"
            name="airtel_account_reference"
            label="Enter account reference"
            onChangeText={(value) =>
              formik.setFieldValue("airtel_account_reference", value)
            }
            value={formik.values.airtel_account_reference}
            inputMode="text"
          />
          <ErrorMessage
            error={formik.errors.airtel_account_reference}
            visible={formik.touched.airtel_account_reference}
          />
        </View>
...
export default PayoutsAirtelScreen;
