...
const PayoutsPaybillScreen = ({ route, navigation }) => {
  const vehicle = route.params;
  console.log("veh at paybill", vehicle);
    ...
    navigation.setOptions({
      ...
      headerTitle: (props) => (
        <Text {...props} style={{ color: "white", ...FONTS.body3 }}>
          Payouts to Paybill - {vehicle.title}
        </Text>
      ),
      headerStyle: {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      paybill_number: Yup.string()
        .min(10, "Too short")
        .required("Paybill number is required"),
      account_number: Yup.string()
        .min(10, "Too short")
        .required("Paybill account number is required"),
      amount: Yup.number().min(1, "Enter 1 or more").required("Amount"),
    }),
    onSubmit: (values, { resetForm }) => {
      console.log("VALZ", values);
      const data = {
        action: "VehicleCollectionToPaybill",
        vehicle_registration: vehicle.title,
        amount: formik.values.amount,
        paybill_number: formik.values.paybill_number,
      ...
      <View>
        ...
        <View className="mt-2">
          <FloatingLabelInput
            ...
            onChangeText={(value) =>
              formik.setFieldValue("paybill_number", value)
            }
            value={formik.values.paybill_number}
            inputMode="text"
          />
          <ErrorMessage
            error={formik.errors.paybill_number}
            visible={formik.touched.paybill_number}
          />
        </View>
...
export default PayoutsPaybillScreen;
